    ...
    public void initialize(URL url, ResourceBundle resourceBundle)  {
        ...
        for (int i = 0; i < crossword.getHeight(); i++) {
            for (int j = 0; j < crossword.getWidth(); j++) {
                boardGrid.add(new CrosswordSquare(crossword,crossword.getCell(i+1, j+1).getSolution(),crossword.getCell(i+1, j+1).getProposition(),"","",crossword.getCell(i+1, j+1).isBlackSquare()), j, i);
            }
        }
